...
function setup() {
  ...
  const service = new TtsProviderConfigService(
    prisma as never,
...
describe("TtsProviderConfigService", () => {
  it("getActiveConfig returns null when no active config exists", async () => {
    const { service, prisma } = setup();
    prisma.ttsProviderConfig.findFirst.mockResolvedValue(null);
  ...
  it("getActiveConfig masks the apiKey", async () => {
    ...
    prisma.ttsProviderConfig.findFirst.mockResolvedValue({
      id: "cfg-1",
      workspaceId: "workspace-1",
      provider: "ELEVENLABS",
      apiKey: encryptionService.encrypt("el-real-key"),
      isActive: true,
    });
    ...
    expect(result?.apiKey).not.toContain("el-real-key");
  });

  it("upsertConfig encrypts the apiKey before persisting", async () => {
    const { service, prisma, encryptionService } = setup();
    prisma.ttsProviderConfig.create.mockImplementation(({ data }) =>
    ...
    const persisted = prisma.ttsProviderConfig.create.mock.calls[0][0].data;
    expect(persisted.apiKey).not.toBe("el-plain-key");
    expect(encryptionService.decrypt(persisted.apiKey)).toBe("el-plain-key");
    expect(persisted.voice).toBe("rachel");
  });
  ...
  it("upsertConfig never returns the plaintext or encrypted apiKey", async () => {
    ...
    expect(result.apiKey).toBe(MASKED_SECRET_VALUE);
    expect(JSON.stringify(result)).not.toContain("el-plain-key");
  });
  ...
  it("upsertConfig deactivates any previously active config when activating a new one", async () => {
    ...
    expect(prisma.ttsProviderConfig.updateMany).toHaveBeenCalledWith({
